...
class RestaurantsDetail:
    ...
    def getRestaurantDetails(self):
        ...
        while True:
            ...
            try:
                self.driver.find_element(By.XPATH, "/html/body/div[2]/section/a").click()
            except:
                break
        list_of_restaurants = self.driver.find_elements(By.XPATH, "//div[@class='storename']")
        for res_url in list_of_restaurants:
            name_and_url = res_url.find_element(By.TAG_NAME, 'a')
            restaurant_urls_list.append(name_and_url.get_attribute('href'))
            restaurant_names_list.append(name_and_url.text)

        location_div = self.driver.find_elements(By.XPATH, "//div[@class='col findus']//div[@class='info']")
        for location in location_div:
            restaurant_location_list.append(location.text.replace("\n", " "))
        number_div = self.driver.find_elements(By.XPATH, "//div[@class='col callus']//div[@class='info']")
        for number in number_div:
            restaurant_number_list.append(number.text)
        for url, name in zip(restaurant_urls_list, restaurant_names_list):
            self.driver.get(url)
            print(f'Scrapping data of "{name}"')
            description_div = self.driver.find_element(By.XPATH, "//div[@class='textbody']")
            restaurant_description_list.append(description_div.text)
